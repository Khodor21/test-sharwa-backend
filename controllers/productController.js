...
const createProduct = async (req, res) => {
  const { title, description, category_id, related_products, pin, main_image, images, quantity, price, target_audience } = req.body;
...
const getAllProducts = async (req, res) => {
  try {
    const products = await Product.find().populate('category_id').populate('related_products');
...
const getProductById = async (req, res) => {
  ...
  try {
    const product = await Product.findById(id).populate('category_id').populate('related_products');
    if (!product) {
